...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Initial bounds: {}x{} at ({}, {})", init_w, init_h, init_x, init_y);
    ...
    println!("\nFinal bounds: {}x{} at ({}, {})", final_w, final_h, final_x, final_y);
    
    // Check if it actually maximized
    let screen_width = 1920; // Typical
    let changed = (final_w - init_w).abs() > 100.0 || (final_h - init_h).abs() > 100.0;
    
    if changed {
        println!("\n✅ SUCCESS: Window bounds changed significantly!");
        println!("   Width: {} → {} (Δ{})", init_w, final_w, final_w as i32 - init_w as i32);
        println!("   Height: {} → {} (Δ{})", init_h, final_h, final_h as i32 - init_h as i32);
    } else {
